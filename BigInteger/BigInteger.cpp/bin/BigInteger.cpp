...
BigInt &BigInt::operator++() { // Changes made in original BigInt and original BigInt returned
    int i, n = digits.size();
...
BigInt BigInt::operator++(int temp) { // Changes made in original BigInt but unchanged 
// copy of original BigInt returned
    BigInt aux; 
    aux = *this;
    ++(*this);
    return aux;
...
BigInt sqrt(BigInt &a) {
    ...
    divide_by_2(right); // As we are considering big integers square root of 'a' 
    // will be 1<=sqrt(a)<=a/2
    while (left <= right) {
        mid += left;
        mid += right;
        divide_by_2(mid);
