...
import Includes.*;
	...
	public class Graph_M 
	{
		...
		// 		Vertex adj=vtces.get(i);
		// 		adj.nbrs.remove(vname);
		// 	}
		// 	vtces.remove(vname);
		// }
		...
		// 	Vertex v=vtces.get(s1);
		// 	if(v.nbrs.containsKey(s2)){
		// 		return true;
		// 	}
		// 	return false;
		// }
		...
		{
			if(!containsEdge(vname1, vname2)){ // This function rund iff there is NOT an edge between the two stations
				Vertex vtx1 = vtces.get(vname1); 
				Vertex vtx2 = vtces.get(vname2); 
		...
		// 		Vertex v1=vtces.get(s1);
		// 		Vertex v2=vtces.get(s2);
		// 		if(v1==null || v2==null){
		// 			return;
		// 		}
		// 		v1.nbrs.remove(s2);
	...
	/////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		// 	// This loop sets the cost of every node as +inf except source and after this it will adjust their costs
		...
		// Here 'nan' is used as a parameter to consider case where minimum time has to be calculated
		public int dijkstra(String src, String des, boolean nan){
			Heap<DijkstraPair> heap=new Heap<>();

			// This hashmap will contains the name of each node and other details like cost and path-so-far (psf) 
			HashMap<String, DijkstraPair> map=new HashMap<>();
			int val=0;
			ArrayList<String> ans=new ArrayList<>();
			...
			while(!heap.isEmpty()){
				DijkstraPair ver=heap.remove();

				if(ver.vname.equals(des)){
					val=ver.cost;
					break;
				}

				map.remove(ver.vname);
				ans.add(ver.vname);
				Vertex k=vtces.get(ver.vname);
				for(String nbr:k.nbrs.keySet()){
					if(map.containsKey(nbr)){
						int old_cost=map.get(nbr).cost;
						int new_cost;
						if(nan){
							new_cost=ver.cost+120+40*k.nbrs.get(nbr);
						}
						else{
							new_cost=ver.cost+k.nbrs.get(nbr);
						}
						if(new_cost<old_cost){
							DijkstraPair dp=map.get(nbr);
							dp.cost=new_cost;
							dp.psf=ver.psf+nbr;
			...
			while(true)
			{
				...
				case 5:
					...
					else 
					{
						...
						// String res[] = g.Get_Minimum_Distance(s1, s2).split("  ");
						int len = str.size();
